...
function get_user_ind(user_mail,userList)
{
  var return_ind
  for(let ind in userList)
  {
    if(userList[ind].email.toLowerCase() ===user_mail.toLowerCase() )
      return_ind=ind
...
buttonChoiseed.forEach(btn=>{
    ...
    (event)=>{
          ...
          //  var signedinCoockie= getcookies("signInUser"); 
           ...
           var ind=get_user_ind(JSON.parse(signedinCoockie),userList)
...
/////////////////////////////
           ...
           tempObject={}
           userList[ind].coursesList=[...enrolled_arr]
          ...
          {
            var returnedCourses=JSON.parse(localStorage.getItem("userList"))[ind].coursesList;
